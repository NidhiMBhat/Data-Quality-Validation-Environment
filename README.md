...
Data quality work is one of the most time-consuming activities in every
organisation that runs data pipelines. Engineers spend 30–40 % of their time
fixing nulls, deduplicating records, standardising formats, and enforcing
referential integrity. This environment lets agents learn to do exactly that,
with deterministic, fully programmatic graders and rich partial-progress
rewards.
...
A 10-row orders table must be validated against a 4-row customers reference.
7 orders contain violations across 4 constraint types:
1. **FK violation** — `customer_id` references a non-existent customer (×2)
2. **Credit exceeded** — `amount` > customer's `credit_limit` (×2)
3. **Non-positive amount** — `amount ≤ 0` (×2)
4. **Bad delivery date** — `delivery_date ≤ order_date` (×1)
...
| Task | Score | Notes |
|------|-------|-------|
| `clean_nulls` | ~0.88 | Occasionally misses one null fill |
| `normalize_formats` | ~0.78 | May struggle with ambiguous date formats |
| `reconcile_tables` | ~0.64 | FK reasoning sometimes requires explicit prompting |
| **Average** | **~0.77** | |
